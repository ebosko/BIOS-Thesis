...
ggplot(heatmap_data_ordinal, aes(x = feature, y = Attribute, fill = mean_score)) +
...
heatmap_data_binary <- binary_data %>%
  group_by(Attribute, feature) %>%
  summarize(mean_score = mean(as.numeric(Value), na.rm = TRUE)) %>%
  ungroup()

# Plot heatmap
ggplot(heatmap_data_binary, aes(x = feature, y = Attribute, fill = mean_score)) +
  geom_tile(color = "white") +
...
tib$Attribute <- factor(tib$Attribute, 
                           levels = c("RUL", "RML", "RLL", "LUS", "LLS", "LLL"))

#fit simple PO model, ignoring REs
tib_polr <- polr(
  Value ~ Attribute + rater, 
  data = tib, 
  method = "logistic", 
  Hess = TRUE
)
...
```{r}
#ensure Value is a factor with levels 0 and 1
ln$Value <- factor(ln$Value, levels = c(0,1))

#make Attribute a factor in the desired order
ln$Attribute <- factor(ln$Attribute, 
                             levels = c("RUL", "RML", "RLL", "LUS", "LLS", "LLL"))

#fit a standard logistic regression model (binary) 
ln_glm <- glm(Value ~ Attribute + rater, 
                 data = ln, 
                 family = binomial(link = "logit"))
...
ggo$Attribute <- factor(ggo$Attribute, 
                           levels = c("RUL", "RML", "RLL", "LUS", "LLS", "LLL"))
...
cons$Attribute <- factor(cons$Attribute, 
                           levels = c("RUL", "RML", "RLL", "LUS", "LLS", "LLL"))
...
bronch$Attribute <- factor(bronch$Attribute, 
                           levels = c("RUL", "RML", "RLL", "LUS", "LLS", "LLL"))
...
atel$Attribute <- factor(atel$Attribute, 
                           levels = c("RUL", "RML", "RLL", "LUS", "LLS", "LLL"))
...
```{r}
#ensure Value is a factor with levels 0 and 1
thin$Value <- factor(thin$Value, levels = c(0,1))

#make Attribute a factor in the desired order
thin$Attribute <- factor(thin$Attribute, 
                             levels = c("RUL", "RML", "RLL", "LUS", "LLS", "LLL"))

#fit a standard logistic regression model (binary) 
thin_glm <- glm(Value ~ Attribute + rater, 
                 data = thin, 
                 family = binomial(link = "logit"))
...
```{r}
#ensure Value is a factor with levels 0 and 1
thick$Value <- factor(thick$Value, levels = c(0,1))

#make Attribute a factor in the desired order
thick$Attribute <- factor(thick$Attribute, 
                             levels = c("RUL", "RML", "RLL", "LUS", "LLS", "LLL"))

#fit a standard logistic regression model (binary) 
thick_glm <- glm(Value ~ Attribute + rater, 
                 data = thick, 
                 family = binomial(link = "logit"))
